import type { Diagnostic, EventHandler, HeartbeatOptions, HostDescriptor, OnEventOptions, PendingRequestInfo, ReliabilityOptions, SdkDebug, SdkEventMap } from './core.types';
...
import type { RpcRequestOptions } from './core.types';
...
export interface MiniAppSdkInterface {
  ...
  once<K extends keyof SdkEventMap>(event: K, options?: OnEventOptions & { signal?: AbortSignal }): Promise<SdkEventMap[K]>;
  once(event: string, options?: OnEventOptions & { signal?: AbortSignal }): Promise<unknown>;
  events<K extends keyof SdkEventMap>(event: K, options?: OnEventOptions & { signal?: AbortSignal }): AsyncIterable<SdkEventMap[K]>;
  events(event: string, options?: OnEventOptions & { signal?: AbortSignal }): AsyncIterable<unknown>;
  request<T>(namespace: string, action: string, payload?: unknown, options?: RpcRequestOptions): Promise<T>;
  requestSafe<T>(namespace: string, action: string, payload?: unknown, options?: RpcRequestOptions): Promise<{ ok: true; value: T } | { ok: false; error: Error }>;
  emit<K extends keyof SdkEventMap>(event: K, data: SdkEventMap[K]): void;
  emit(event: string, data?: unknown): void;
  use(middleware: (ctx: { namespace: string; action: string; payload: unknown; attempt: number }, next: () => Promise<unknown>) => Promise<unknown>): void;
  addEventInterceptor(interceptor: (event: string, payload: unknown) => unknown | false): () => void;
  getMetrics(): { totalRequests: number; totalSuccesses: number; totalFailures: number; totalTimeouts: number; totalRetries: number; averageDurationMs: number; percentiles: { p50Ms: number; p95Ms: number; p99Ms: number }; byAction: Record<string, unknown> };
  ...
  registerLazyModule<T>(name: string, factory: () => Promise<(rpc: any) => T> | ((rpc: any) => T)): void;
  batch(requests: Array<{ namespace: string; action: string; payload?: unknown; options?: RpcRequestOptions }>): Promise<Array<{ ok: true; value: unknown } | { ok: false; error: Error }>>;
  usePlugin(plugin: SdkPlugin): Promise<void>;
}

export interface SdkPlugin {
  name: string;
  install(ctx: { sdk: MiniAppSdkInterface; rpc: any; logger: any }): void | Promise<void>;
...
export interface MiniAppSdkOptions {
  ...
  metrics?: { maxDurationEntries?: number; durationsWindowMs?: number; onSnapshot?: (snapshot: unknown) => void };
